...
exports.parse = function(userAgent) {
  ...
  if(osId) {
    ...
    if (os.osFamily === os.osName) {
      // Take first set of brackets
      var name = userAgent.match(/\(([^)]+)\)/)[0];

      // If only one semi-colon, most user agents reference 
      if (name.match('Trident')) {
        if (name.match('MSIE 10.0 Windows; Trident/4.0')) {
          name = 'Windows';
          os.osName = 'Windows';
        }
        if (name.match('MSIE 10.0; Windows; Trident/5.0')) {
          name = 'Windows';
          os.osName = 'Windows';
        }
        if (name.match('MSIE 10.0; Windows; Trident/6.0')) {
          name = 'Windows';
          os.osName = 'Windows';
        }
        if (name.match('MSIE 10.0; Windows; Trident/7.0')) {
          name = 'Windows';
          os.osName = 'Windows';
        }
      }
      if (name.match('Macintosh')) {
        name = name.split(';')[1];
      }
      if ((name.split(';').length -1) === 1) {
        name = name.substr(0, name.indexOf(';'));
        os.osName = name.slice(1);
      } else {
        if (name.indexOf(';') !== -1) {
          if (name.split(';').length > 1) {
            os.osName = name.match('\;([^;]*)')[0].replace('; ', '');
          } else {
            os.osName = name.substring(1, name.indexOf(';'));
          }
        } else {
          if (name.indexOf(')') !== -1) {
            os.osName = name.substring(name.indexOf(')'), -1);
          }
        }
      }
    }
